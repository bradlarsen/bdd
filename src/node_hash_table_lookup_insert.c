...
double_hash_table_num_buckets (node_hash_table_t *tab)
{
    ...
    node_bucket_t **old_buckets = tab->buckets;

    tab->num_buckets *= 2;
    tab->buckets =
        (node_bucket_t **) malloc (tab->num_buckets * sizeof(node_bucket_t *));
    for (unsigned i = 0; i < tab->num_buckets; i += 1)
        tab->buckets[i] = NULL;
    tab->num_entries = 0;

    for (unsigned i = 0; i < old_num_buckets; i += 1) {
        for (node_bucket_t *p = old_buckets[i]; p != NULL; p = p->next) {
            node_hash_table_insert (tab, p->key, p->value);
        }
        node_bucket_free (old_buckets[i]);
    }
    free (old_buckets);
                        ...
                        unsigned val)
{
    ...
    node_bucket_t *b_node = node_bucket_search (tab->buckets[b_idx], key);
    if (b_node == NULL) {
        tab->buckets[b_idx] = node_bucket_create (key, val, tab->buckets[b_idx]);
        tab->num_entries += 1;
    }
    else {
        b_node->value = val;
