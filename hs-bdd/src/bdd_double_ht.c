...
/***********************************************************************/
#define bdd_double_ht_check_invariants(tab)                 \
    do {                                                \
        assert (tab != NULL);                           \
        assert (tab->buckets != NULL);                  \
        assert (bdd_double_ht_proper_hash_values(tab));     \
        assert (bdd_double_ht_no_duplicate_keys(tab));      \
    } while (0)
 ...
 * bucket. */
static bool
bdd_double_ht_proper_hash_values (bdd_double_ht_t *tab)
{
    unsigned i;
    ht_bucket_t *p;
    for (i = 0; i < tab->num_buckets; i += 1)
        for (p = tab->buckets[i]; p != NULL; p = p->next)
            if (bdd_double_ht_get_hash_index(tab, p->key) != i)
                return false;
    return true;
}
 ...
 * the same key. */
static bool
bdd_double_ht_no_duplicate_keys (bdd_double_ht_t *tab)
{
    unsigned num_buckets;
    ht_bucket_t **buckets;
...
{
    return bdd_double_ht_create_with_hint (32);
}
...
bdd_double_ht_create_with_hint (unsigned num_buckets_hint)
{
    ...
    for (i = 0; i < num_buckets; i += 1)
        tab->buckets[i] = NULL;
    bdd_double_ht_check_invariants (tab);
    return tab;
}
...
bdd_double_ht_get_load (bdd_double_ht_t *tab)
{
    bdd_double_ht_check_invariants (tab);
    return
        (float) bdd_double_ht_get_num_entries (tab) /
        (float) bdd_double_ht_get_num_buckets (tab);
}
