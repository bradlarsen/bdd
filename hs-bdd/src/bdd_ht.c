...
/* HASH TABLE BUCKETS                                                  */
/***********************************************************************/
typedef struct bdd_ht_bucket
{
    raw_bdd_t key;
    raw_bdd_t value;
    struct bdd_ht_bucket *next;
} bdd_ht_bucket_t;

/* Allocates and initializes a bucket with the given parameters. */
static bdd_ht_bucket_t *
bdd_ht_bucket_create (raw_bdd_t key,
                            raw_bdd_t value,
                            bdd_ht_bucket_t *next)
{
    bdd_ht_bucket_t * bucket = (bdd_ht_bucket_t *)
        checked_malloc (sizeof(bdd_ht_bucket_t));
    bucket->key = key;
    bucket->value = value;
    bucket->next = next;
    return bucket;
...
static void
bdd_ht_bucket_free (bdd_ht_bucket_t *bucket)
{
    bdd_ht_bucket_t *p = bucket;
    while (p != NULL) {
        bdd_ht_bucket_t *prev = p;
        p = p->next;
        checked_free (prev);
    }
}
...
static bdd_ht_bucket_t *
bdd_ht_bucket_search (bdd_ht_bucket_t *bucket, raw_bdd_t key)
{
    bdd_ht_bucket_t *p = bucket;
    while (p != NULL) {
 ...
 * it. */
static unsigned
bdd_ht_get_hash_index (bdd_ht_t *tab, raw_bdd_t key)
{
...
bdd_ht_create ()
{
    return bdd_ht_create_with_hint (32);
}
...
bdd_ht_create_with_hint (unsigned num_buckets_hint)
{
    ...
    bdd_ht_t *tab = (bdd_ht_t *)
        checked_malloc (sizeof(bdd_ht_t));
    tab->num_entries = 0;

    tab->num_buckets = num_buckets;
    tab->buckets = (bdd_ht_bucket_t **)
        checked_malloc (num_buckets * sizeof(bdd_ht_bucket_t *));
...
bdd_ht_destroy (bdd_ht_t *tab)
{
    ...
    for (i = 0; i < tab->num_buckets; i += 1)
        bdd_ht_bucket_free (tab->buckets[i]);
...
static void
double_hash_table_num_buckets (bdd_ht_t *tab)
{
    bdd_ht_bucket_t **old_buckets;
    ...
    tab->num_buckets *= 2;
    tab->buckets = (bdd_ht_bucket_t **)
        checked_malloc (tab->num_buckets * sizeof(bdd_ht_bucket_t *));
    for (i = 0; i < tab->num_buckets; i += 1)
        tab->buckets[i] = NULL;
    tab->num_entries = 0;
    ...
    for (i = 0; i < old_num_buckets; i += 1) {
        for (p = old_buckets[i]; p != NULL; p = p->next) {
            bdd_ht_insert (tab, p->key, p->value);
        }
        bdd_ht_bucket_free (old_buckets[i]);
    }
                  ...
                  raw_bdd_t val)
{
    ...
    b = bdd_ht_bucket_search (tab->buckets[b_idx], key);
    if (b == NULL) {
        tab->buckets[b_idx] = bdd_ht_bucket_create (key, val, tab->buckets[b_idx]);
        tab->num_entries += 1;
    }
    else
        b->value = val;
...
raw_bdd_t *
bdd_ht_lookup (bdd_ht_t *tab, raw_bdd_t key)
{
    unsigned b_idx;
    bdd_ht_bucket_t *b;
...
{
    unsigned idx;
    for (idx = 0; idx < bdd_ht_get_num_buckets (tab); idx += 1) {
        bdd_ht_bucket_t *p = tab->buckets[idx];
        while (p != NULL) {
