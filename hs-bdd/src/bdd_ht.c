...
bdd_ht_bucket_create (bdd_t key,
                            bdd_t value,
...
static bdd_ht_bucket_t *
bdd_ht_bucket_search (bdd_ht_bucket_t *bucket, bdd_t key)
{
    bdd_ht_bucket_t *p = bucket;
    while (p != NULL) {
 ...
 * it. */
static unsigned
bdd_ht_get_hash_index (bdd_ht_t *tab, bdd_t key)
{
...
bdd_t *
bdd_ht_lookup (bdd_ht_t *tab, bdd_t key)
{
    unsigned b_idx;
    bdd_ht_bucket_t *b;
